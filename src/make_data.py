...
if __name__ == '__main__':
    ...
    if args.task == 'df': # and valid_df
        ...
        with open(f'{RAW_DATA_DIR}/extractive_test_v1.jsonl', 'r') as json_file:
        ...
        #df['extractive_sents'] = df.apply(lambda row: [row['article_original'][i] for i in row['extractive'] if i is not None] , axis=1)

        # random split
        train_df = df.sample(frac=0.7,random_state=42) #random state is a seed value
    ...
    # Make bert input file for test from df file
        ...
        ## make json file
        # 동일한 파일명 존재하면 덮어쓰는게 아니라 ignore됨에 따라 폴더 내 삭제 후 만들어주기
        json_data_dir = f"{JSON_DATA_DIR}/test_{args.target_summary_sent}" #json_data_dir = f"{JSON_DATA_DIR}/test_ext"
        if os.path.exists(json_data_dir):
            os.system(f"rm {json_data_dir}/*")
        else:
            os.mkdir(json_data_dir)

        #create_json_files(test_df, data_type='test', target_summary_sent='ext', path=JSON_DATA_DIR)
        create_json_files(test_df, data_type='test',target_summary_sent=args.target_summary_sent, path=JSON_DATA_DIR)
        
        ## Convert json to bert.pt files
        bert_data_dir = f"{BERT_DATA_DIR}/test_ext" #/test_ext
        if os.path.exists(bert_data_dir):
            os.system(f"rm {bert_data_dir}/*")
        else:
            os.mkdir(bert_data_dir)
